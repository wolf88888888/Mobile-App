...
import {getCurrencyRates, getPropertyById} from '../../utils/requester';

class SmallPropertyType extends Component {
  constructor(props) {
    super(props);
    this.state = {
      property: null,
  ...
  async componentDidMount() {
    ...
    this.setState({ property, currencyConversion });
  }

  renderReviewText() {
    const averageRating = this.state.property.averageRating;
  ...
  render() {
    ...
    const { prices, currencyCode, getDailyPrice, pictures, averageRating, hotelPhotos, star } = listing;
    ...
    if (listingsType === 'homes') {
      ...
      listingPrice = 0.0;
      photos = hotelPhotos.map(x => { return {thumbnail: 'http://roomsxml.com' + x.externalThumbnailUrl}; });
      rating = star;
    }
...
SmallPropertyType.propTypes = {
  listing: PropTypes.object
};

export default connect(mapStateToProps)(SmallPropertyType);
